...
Route::middleware(['web', 'auth'])->group(function () {
    ...
    // --- ROUTE DUYỆT BÀI (Phiên bản Fix lỗi 500) ---
        Route::post('/approve-entity/{type}/{id}', function (Request $request, $type, $id) {
            ...
            // Check quyền Admin thủ công qua bảng role_user
            $userRoleIds = DB::table('role_user')->where('user_id', $userId)->pluck('role_id')->toArray();
            $isAdmin = in_array(1, $userRoleIds); 
            $isBookOwner = ((int)$userId === (int)$bookOwnerId);
            ...
            if ($isAdmin || $isBookOwner) {
                ...
                // B. Ghi Log Hoạt Động (Phần gây lỗi cũ đã được bọc lại an toàn)
                try {
                    // Tìm đối tượng Model chuẩn của BookStack
                    $model = null;
                    if ($type === 'page') {
                    ...
                    if ($model) {
                        // SỬA LỖI QUAN TRỌNG:
                        // 1. Dùng \BookStack\Facades\Activity (Đường dẫn tuyệt đối)
                        // 2. Dùng string 'entity_approved' trực tiếp (Tránh lỗi thiếu hằng số ActivityType)
                        \BookStack\Facades\Activity::add(
                            'entity_approved', // Key hoạt động
                            $model,            // Tham số 2: Link
                            $model             // Tham số 3: Tên (Detail)
                        );
                    }
                } catch (\Exception $e) {
                    // Nếu lỗi ghi log thì bỏ qua, không làm sập trang web
                    \Illuminate\Support\Facades\Log::error("Lỗi Activity Log: " . $e->getMessage());
                }
